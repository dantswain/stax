...
pub async fn run() -> Result<()> {
    ...
    if let Some(current_branch) = stack.branches.get(&stack.current_branch) {
        if let Some(pr) = &current_branch.pull_request {
            println!("Pull Request: {} ({})", pr.html_url.blue(), pr.state.yellow());
        }
        ...
        if !current_branch.children.is_empty() {
            println!("Child branches: {}", current_branch.children.join(", ").cyan());
        }
    }
    ...
    let has_upstream_branches = stack.branches.values()
